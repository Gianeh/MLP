...
# Experiments on Boston Housing dataset
...
if __name__ == "__main__":
    ...
    # Data splitting
    data_X_train, data_y_train, data_X_val, data_y_val, data_X_test, data_y_test = create_splits_unbalanced(data_X, data_y, train_val_fractions=[0.7, 0.15], randomize=True)
    ...
    # Training phase
    net.train(data_X_train, data_y_train, batch_size=0, epochs=1000, lr=0.00005, X_Val=data_X_val, Y_Val=data_y_val, plot=True, optimizer="adam")
    
    # Saving the model and the loss plot
    net.plot(save=True)
    net.save_model("housing_model")
